...
class Enemy:
    def __init__(self, x, y, width, height, end):
        ...
        self.width = width
        self.height = height
        self.end = screenWidth - self.width
        self.start = 0
        self.step = 6
        self.moves = 0
...
enemy_disappeared = False

def redrawGame():
    global enemy_disappeared
    global moves
    ...
    if not enemy.visible :
        ...
        screen.blit(text_win, (screenWidth // 2 - text_win.get_width() // 2, screenHeight // 2 - text_win.get_height() // 2))
        enemy_disappeared = True
    if score < -5:
        font_game_over = pygame.font.SysFont("comicsans", 60)
        text_game_over = font_game_over.render("Game Over", 1, RED)
        screen.blit(text_game_over, (screenWidth // 2 - text_game_over.get_width() // 2, 200))
        pygame.display.update()

        # Attendre 3 secondes avant de quitter le jeu
